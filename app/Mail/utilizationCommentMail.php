...
class utilizationCommentMail extends Mailable
{
    ...
    public function build()
    {
        ...
        $get_email = DB::table('db_utilization')
        ->leftjoin('users', 'db_utilization.users_id', '=', 'users.idCard')
        ->select('db_utilization.id',
                 'users.email',
                )
        ->where('db_utilization.id',$this->data['utilization_id'])
        ->first();
        ...
        return $this
        ->to($get_email->email)
        ->view('emails.utilizationCommentMail')
